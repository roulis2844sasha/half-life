...
public hook_say(id)
{
	...
	if(get_user_flags(id) & NICK_LEVEL && get_pcvar_num(g_AdminPrefix))
	{
		...
		if(get_user_flags(id) & ADMIN_LEVEL_D && get_pcvar_num(g_AdminPrefix))
	{
		mLen += format(Message[mLen], charsmax(Message) - mLen, "[^x04%L^x01] ", LANG_PLAYER, "CT_VIP")
		lgLen += format(p_LogMsg[lgLen], charsmax(p_LogMsg) - lgLen, "<font color=^"green^">[%L] </font>", LANG_PLAYER, "CT_VIP")
	}
	if(get_user_flags(id) & ADMIN_LEVEL_G && get_pcvar_num(g_AdminPrefix))
	{
		mLen += format(Message[mLen], charsmax(Message) - mLen, "[^x04%L^x01] ", LANG_PLAYER, "CT_CHIEF")
		lgLen += format(p_LogMsg[lgLen], charsmax(p_LogMsg) - lgLen, "<font color=^"green^">[%L] </font>", LANG_PLAYER, "CT_CHIEF")
	}
	else if(get_user_flags(id) & ADMIN_USER && get_pcvar_num(g_AdminPrefix))
	{
		mLen += format(Message[mLen], charsmax(Message) - mLen, "[^x04%L^x01] ", LANG_PLAYER, "CT_PLAYER")
		lgLen += format(p_LogMsg[lgLen], charsmax(p_LogMsg) - lgLen, "<font color=^"green^">[%L] </font>", LANG_PLAYER, "CT_PLAYER")
	}
